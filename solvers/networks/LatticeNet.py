...
class CoeffConv(nn.Module):
    def __init__(self, num_fea):
        super(CoeffConv, self).__init__()
        r = 16
        self.upper_branch = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Conv2d(num_fea, num_fea // r, 1, 1, 0),
            nn.ReLU(inplace=True),
            nn.Conv2d(num_fea // r, num_fea, 1, 1, 0),
            nn.Sigmoid()
        )
...
class LBlock(nn.Module):
    def __init__(self, num_fea):
        ...
        self.A1_coeff_conv = CoeffConv(num_fea)
        self.B1_coeff_conv = CoeffConv(num_fea)
        ...
        self.A2_coeff_conv = CoeffConv(num_fea)
        self.B2_coeff_conv = CoeffConv(num_fea)
    ...
    def forward(self, x):
        H = self.H_conv(x)
        A1 = self.A1_coeff_conv(H)
        P1 = x + A1*H
        B1 = self.B1_coeff_conv(x)
        Q1 = H + B1*x

        G = self.G_conv(P1)
        B2 = self.B2_coeff_conv(G)
        Q2 = Q1 + B2*G
        A2 = self.A2_coeff_conv(Q1)
        P2 = G + A2*Q1
