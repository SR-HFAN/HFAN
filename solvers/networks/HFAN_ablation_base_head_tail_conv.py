...
class HFAN_ablation_base_head_tail_conv(nn.Module):
    def __init__(self, upscale_factor=2, in_channels=3, num_fea=64, out_channels=3, down_RBs=4, up_RBs=2, reduce_rate=4):
        ...
        # transition
        self.transition = nn.Conv2d(num_fea, num_fea, 3, 1, 1)
...
# 600K, 34.5G
s = 4
m = HFAN_ablation_base_head_tail_conv(upscale_factor=s).to('cuda')
