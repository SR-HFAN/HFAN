...
from .blocks import MeanShift, ESA
...
class BaselineESA(nn.Module):
    def __init__(self, upscale_factor=2, in_channels=3, num_fea=46, out_channels=3, down_RBs=15):
        super(BaselineESA, self).__init__()
        ...
        for i in range(down_RBs):
            ...
            if i % 3 == 0:
                att = ESA(num_fea)
            RBs.append(ResBlock(num_fea=num_fea, att=att))
...
# 622K, 34.6G
s = 4
m = BaselineESA(upscale_factor=s).to('cuda')
