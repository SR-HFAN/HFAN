...
class SRSolver(BaseSolver):
    def __init__(self, opt):
        ...
        self.lg.info('use split forward in validation phase: [{}]'.format(self.use_chop))
        ...
        # get parameters and GFlops
        params, GFlops = self.count_parameters_GFlops()
        self.lg.info('Total parameters: [{:.3f}M], GFlops: [{:.4f}G]'.format(params / 1e6, GFlops / 1e9))
        ...
        if optim_type == 'ADAM':
            self.optimizer = optim.Adam(filter(lambda p: p.requires_grad, self.model.parameters()), lr=lr, betas=(self.train_opt['beta1'], self.train_opt['beta2']), eps=self.train_opt['eps'], weight_decay=weight_decay)
        elif optim_type == 'SGD':
            self.optimizer = optim.SGD(filter(lambda p: p.requires_grad, self.model.parameters()), lr=lr, momentum=self.train_opt['momentum'], weight_decay=weight_decay)
        else:
    ...
    def test(self):
        ...
        # For x3 resolution, SR and HR may be not the same resolution
        h, w = self.SR.shape[2:]
    ...
    def load(self):
        ...
        if resume is None:
            ...
            self.lg.info('Resume from [{}]...'.format(resume))
    ...
    def save_checkpoint(self, epoch, is_best):
        last_filename = osp.join(self.ckp_path, 'last_ckp.pth')
        ckp = {
        ...
        torch.save(ckp, last_filename)
        self.lg.info('Saving last checkpoint to [{}]'.format(last_filename))
